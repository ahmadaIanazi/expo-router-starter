 ...
 */
export function Main({ s, ref, children, color, shadowColor, borderColor }) {
  const themeColor = useContext(themeContext);
  const l = useContext(LOCALIZATION)
  const styleColor = {
    shadowColor: shadowColor || themeColor.shadow || null,
    borderColor: borderColor || themeColor.border || null,
  };
  const labelStyle = applyStyles(styleColor, s);
  const backgroundColor = color || null;

  return (
    <View ref={ref} style={[labelStyle, { backgroundColor: backgroundColor, direction: l.direction }]}>
      {children}
    </View>
  );
}
 ...
 *   - color: string (optional) - Text Color
 */
export function Y({ s, ref, children, color, shadowColor, borderColor }) {
  const themeColor = useContext(themeContext);
  const l = useContext(LOCALIZATION)
  const styleColor = {
    shadowColor: shadowColor || themeColor.shadow || null,
 ...
 *   - onPress: function (optional) - Callback function for button press event
 *   - disabled: boolean (optional) - Indicates if the button is disabled
 *   - color: string (optional) - Text Color
 */
export function Tap({ s, onPress, style = true, disabled, borderColor, children, color, refer }) {

  const themeColor = useContext(themeContext);
  const l = useContext(LOCALIZATION)
  const defaultStyle = style
    ? {
        fontSize: isTablet ? WIDTH_RATIO * 14 : 14,
        fontWeight: 'bold',
      }
    : null;
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        labelStyle,
        { backgroundColor: backgroundColor, borderColor: borderColor || themeColor.border  },
      ]}
    >
      {children}
    </TouchableOpacity>
 ...
 */
export function Button({ s, ref, title, onPress, disabled, color, textColor, icon, iconColor, iconSize, family, row = true  }) {
  const themeColor = useContext(themeContext);
  const labelStyle = applyStyles(styles.label, s);
  const buttonColor = color || themeColor.primary;
  const buttonTextColor = textColor || themeColor.text || 'grey';
  const handleEmpty = () => {};
  const handler = onPress || handleEmpty();
  const iconName = icon || null 
  const iconSizeSet = iconSize || 24
  const iconFamily = family || null 
  const iconColorSet = iconColor || textColor || null

  return (
    <TouchableOpacity ref={ref} onPress={handler} disabled={disabled}>
      <View style={[styles.buttonContainer, labelStyle, { backgroundColor: buttonColor, flexDirection: row? 'row' : null }]}>
    {iconName != null && <Icon s='me-10' name={iconName} size={iconSizeSet} color={iconColorSet} family={iconFamily} />}
        <T s='ts-14 b' color={buttonTextColor}>{title}</T>
      </View>
    </TouchableOpacity>
  );
}
...
export function Overlay({ s, ref, style = true, color, opacity = 0.6, shadowColor, borderColor }) {
  ...
  const labelStyle = applyStyles(styleColor, s);
  const backgroundColor = color || themeColor.accent || 'white';
 ...
 *   - colors: string (optional) - Text Color
 *   - size: integer (optional) - Icon Size
 */
export function ShadeBottom({ s, ref, colors, zIndex = 1, height = 200, style = true }) {
  const themeColor = useContext(themeContext);
  const styleShade = style
    ? {
        zIndex: zIndex,
        bottom: 0,
        position: 'absolute',
    ...
    : null;
  const labelStyle = applyStyles(styleShade, s);
  const shade = colors || [themeColor.transparent, themeColor.background] || [
      'transparent',
      'grey',
    ];

  return <LinearGradient ref={ref} colors={shade} style={labelStyle} />;
}
...
export function Input({
  ...
  const styleColor = style
    ? {
        ...
        shadowColor: shadowColor || themeColor.shadow || null,
        borderColor: borderColor || themeColor.border || null,
        backgroundColor: themeColor.light || 'grey',
      }
    : null;
  const labelStyle = applyStyles(styleColor, s);
  const backgroundColor = color || null;
  const placeholderContent = placeholder || '';
  const placeholderColorContent = placeholderColor || themeColor.placeholder || 'lightgrey';
  const onChangeText = onChange || null;
  const inputValue = value || null;
  const inputTextColor = textColor || themeColor.text || 'black';
 ...
 *   - textColor: string (optional) - Button text color
 */
export function BigButton({ s, ref, title, onPress, backgroundColor, textColor, loading = false }) {
  const themeColor = useContext(themeContext);
  const labelStyle = applyStyles(styles.label, s);
  const buttonBackgroundColor = backgroundColor || themeColor.primary || 'grey';
  const buttonTextColor = textColor || themeColor.contrast || 'grey';

  return (
    <TouchableOpacity ref={ref} onPress={onPress} disabled={loading}>
      <View
        style={[styles.bigActionButton, labelStyle, { backgroundColor: buttonBackgroundColor }]}
      >
        {loading ? (
          <ActivityIndicator color={buttonTextColor} />
        ) : (
          <Text style={[styles.bigActionButtonText, { color: buttonTextColor }]}>{title}</Text>
        )}
      </View>
    </TouchableOpacity>
  );
